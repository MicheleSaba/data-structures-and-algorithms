...
const listFoods = (recipe) => {
  ...
  for (let i = 0; i < recipe.ingredients.length; i++){
    let removeText1 = recipe.ingredients[i].slice(recipe.ingredients[i].indexOf(' ')+ 1);
    var removeText2 = removeText1.slice(removeText1.indexOf(' ') + 1);
      result.push(removeText2)
   }
...
const splitFoods = (recipe) => {
  let result = [];
  for (let i = 0; i < recipe.ingredients.length; i++){
    var ingredients = (str.split(recipe.ingredients[i], [2]));
      result.push(str.split(recipe.ingredients[i], [2]));
   }
  // Solution code here...
  return result;
};
