...
const getStatName = (arr, minBaseStat) => {
  return arr.filter(stat => stat.baseStat > minBaseStat).map(
      object => object.stat.name);
};
...
const getCharactersWithoutChildren = (arr) => {
   return arr.filter(object => !object.hasOwnProperty('children'));
};
...
const evenOddNumericValues = (arr) => {
    return arr.filter(word => !isNaN(word) && typeof word !== 'string').map(
        num => {if (num % 2 === 0) {
          return 'even';
        }
        else {
